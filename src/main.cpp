...
#include "editor/panels/CombatDebugPanel.h"
#include "editor/panels/InspectorSectionsPanel.h"
#include "editor/panels/MaterialBrowserPanel.h"
#include "editor/panels/PerformancePanel.h"
#include "editor/panels/PhysicsPanel.h"
...
int main(int argc, char** argv)
{
    ...
    // Play/edit mode snapshot/session state is handled by the editor session helper.
    MyEngine::Editor::PlayModeSession playModeSession;
    ...
    MyEngine::Editor::Panels::InspectorSectionState inspectorSectionState;
    ...
    auto& drawCombatShapesOverlay = editorUI.drawCombatShapesOverlay;
    ...
    MyEngine::Editor::PlayModeRestoreContext playModeRestoreContext;
    playModeRestoreContext.scene = &scene;
    playModeRestoreContext.selectedEntity = &selectedEntity;
    playModeRestoreContext.playerEntity = &playerEntity;
    playModeRestoreContext.undoStack = &undoStack;
    playModeRestoreContext.defaultShader = litShader;
    playModeRestoreContext.globalScripts = &globalScripts;
    ...
    auto setPlaying = [&](bool play)
    {
        ...
        if (play)
        {
            playModeSession.CaptureSnapshot(scene, globalScripts);
            isPlaying = true;
            ...
            if (networkingEnabled && playModeSession.HasSnapshot())
            {
                MyEngine::Serialization::LoadSceneFromString(authoritativeServerScene, playModeSession.GetSnapshotJson(), litShader, &globalScripts);
        ...
        else
        {
            ...
            hasAuthoritativeServerScene = false;
#ifdef USE_IMGUI
            if (playModeSession.HasSnapshot())
            {
                playModeSession.RequestStopPrompt();
                return;
            }
#endif
            playModeSession.RestoreSnapshot(playModeRestoreContext);
        }
    };
    ...
    while (!renderPlatform->ShouldClose())
    {
        ...
        MyEngine::Editor::StopPlayPromptContext stopPlayPromptContext;
        stopPlayPromptContext.isPlaying = &isPlaying;
        stopPlayPromptContext.scene = &scene;
        stopPlayPromptContext.globalScripts = &globalScripts;
        stopPlayPromptContext.recentScenes = &recentScenes;
        stopPlayPromptContext.addRecentScene = AddRecentScene;
        stopPlayPromptContext.restoreContext = playModeRestoreContext;
        playModeSession.DrawStopPlayPrompt(stopPlayPromptContext);
        ...
        if (showUI)
        {
            ...
            if (showInspector)
            {
                ...
                if (selectedEntity)
                {
                    MyEngine::Editor::Panels::InspectorSectionsContext inspectorSectionsContext;
                    inspectorSectionsContext.scene = &scene;
                    inspectorSectionsContext.selectedEntity = selectedEntity;
                    inspectorSectionsContext.undoStack = &undoStack;
                    inspectorSectionsContext.state = &inspectorSectionState;
                    inspectorSectionsContext.selectedAnimationStateMachinePath = &selectedAnimationStateMachinePath;
                    inspectorSectionsContext.editingAnimationStateMachine = &editingAnimationStateMachine;
                    inspectorSectionsContext.describePrefabOverrides = describePrefabOverrides;
                    inspectorSectionsContext.revertSelectedPrefabOverride = revertSelectedPrefabOverride;
                    inspectorSectionsContext.applyAllPrefabOverrides = applyAllPrefabOverrides;
                    MyEngine::Editor::Panels::DrawInspectorPrefabAndEntitySections(inspectorSectionsContext);
                    MyEngine::Editor::Panels::DrawInspectorTransformAndCameraSections(inspectorSectionsContext);
                    ...
                    MyEngine::Editor::Panels::DrawInspectorPhysicsSections(inspectorSectionsContext);

                    MyEngine::Editor::Panels::DrawInspectorCombatSections(inspectorSectionsContext);

                    // Joint Component (Fixed/Spring/Hinge constraints between entities)
                    MyEngine::Editor::Panels::DrawInspectorJointSections(inspectorSectionsContext);

                    // Animation Component
                    MyEngine::Editor::Panels::DrawInspectorAnimationSections(inspectorSectionsContext);

                    MyEngine::Editor::Panels::DrawInspectorAudioSections(inspectorSectionsContext);

                    MyEngine::Editor::Panels::DrawInspectorScriptSections(inspectorSectionsContext);

                    MyEngine::Editor::Panels::DrawInspectorParticleSections(inspectorSectionsContext);
                    MyEngine::Editor::Panels::DrawInspectorAudioListenerSections(inspectorSectionsContext);
                }
                    else
                    {
            ...
            MyEngine::Editor::Panels::PerformancePanelContext performancePanelContext;
            performancePanelContext.isOpen = &showPerformancePanel;
            performancePanelContext.scene = &scene;
            performancePanelContext.renderSystem = &renderSystem;
            performancePanelContext.physicsSystem = &physicsSystem;
            performancePanelContext.networkTransport = networkTransport.get();
            performancePanelContext.pluginManager = &pluginManager;
            performancePanelContext.loadPlugins = [&]()
            {
                MyEngine::Net::NetPluginRegistry::Clear();
                pluginManager.LoadAllFromRoot(pluginRootPath, pluginHostApi);
            };
            performancePanelContext.reloadPlugins = [&]()
            {
                MyEngine::Net::NetPluginRegistry::Clear();
                pluginManager.ReloadAllFromRoot(pluginRootPath, pluginHostApi);
            };
            performancePanelContext.unloadPlugins = [&]()
            {
                pluginManager.UnloadAll();
                MyEngine::Net::NetPluginRegistry::Clear();
            };
            performancePanelContext.exportPerformanceCsv = exportPerformanceCsv;
            performancePanelContext.deltaTime = deltaTime;
            performancePanelContext.gpuFrameMs = gpuFrameMs;
            performancePanelContext.cpuPhysicsMs = cpuPhysicsMs;
            performancePanelContext.cpuAnimationMs = cpuAnimationMs;
            performancePanelContext.cpuRenderMs = cpuRenderMs;
            performancePanelContext.replayParticleSeed = replayParticleSeed;
            performancePanelContext.framePacingDiagnostics = &framePacingDiagnostics;
            performancePanelContext.pendingTerrainPatchJobCount = pendingTerrainPatchJobs.size();
            performancePanelContext.terrainPatchQueuePeak = terrainPatchQueuePeak;
            performancePanelContext.terrainPatchPrepMs = terrainPatchPrepMs;
            performancePanelContext.terrainPatchUploadMs = terrainPatchUploadMs;
            performancePanelContext.terrainPatchBackpressureDeferrals = terrainPatchBackpressureDeferrals;
            performancePanelContext.terrainPatchMaxQueuedJobs = &terrainPatchMaxQueuedJobs;
            performancePanelContext.navMeshRebuildMs = navMeshRebuildMs;
            performancePanelContext.navMeshLastRebuildWasRegion = navMeshLastRebuildWasRegion;
            performancePanelContext.memoryWorkingSetMB = memoryWorkingSetMB;
            performancePanelContext.memoryPrivateMB = memoryPrivateMB;
            performancePanelContext.frameMsHistory = frameMsHistory.data();
            performancePanelContext.physicsMsHistory = physicsMsHistory.data();
            performancePanelContext.animationMsHistory = animationMsHistory.data();
            performancePanelContext.renderMsHistory = renderMsHistory.data();
            performancePanelContext.visibleCountHistory = visibleCountHistory.data();
            performancePanelContext.occlusionRejectHistory = occlusionRejectHistory.data();
            performancePanelContext.frustumRejectHistory = frustumRejectHistory.data();
            performancePanelContext.perfHistoryCount = frameMsHistory.size();
            performancePanelContext.perfHistoryIndex = perfHistoryIndex;
            performancePanelContext.networkSessionBootstrapPending = networkSessionBootstrapPending;
            performancePanelContext.usingSocketTransport = usingSocketTransport;
            performancePanelContext.localClientID = localClientID;
            performancePanelContext.networkTick = networkTick;
            performancePanelContext.hasAuthoritativeServerScene = hasAuthoritativeServerScene;
            performancePanelContext.transportInputDelayTicks = &transportInputDelayTicks;
            performancePanelContext.transportSnapshotDelayTicks = &transportSnapshotDelayTicks;
            performancePanelContext.networkClientSmoothingEnabled = &networkClientSmoothingEnabled;
            performancePanelContext.networkClientPositionSmoothingRate = &networkClientPositionSmoothingRate;
            performancePanelContext.networkClientRotationSmoothingRate = &networkClientRotationSmoothingRate;
            performancePanelContext.networkClientPositionSnapDistance = &networkClientPositionSnapDistance;
            performancePanelContext.networkClientRotationSnapDistance = &networkClientRotationSnapDistance;
            performancePanelContext.networkInterestSettings = &networkInterestSettings;
            performancePanelContext.viewportWidth = g_WindowWidth;
            performancePanelContext.viewportHeight = g_WindowHeight;
            performancePanelContext.wireframe = &wireframe;
            MyEngine::Editor::Panels::DrawPerformancePanel(performancePanelContext);

            MyEngine::Editor::Panels::PhysicsPanelContext physicsPanelContext;
            physicsPanelContext.isOpen = &showPhysicsPanel;
            physicsPanelContext.isPlaying = &isPlaying;
            physicsPanelContext.physicsSystem = &physicsSystem;
            physicsPanelContext.setPlaying = setPlaying;
            MyEngine::Editor::Panels::DrawPhysicsPanel(physicsPanelContext);

            MyEngine::Editor::Panels::CombatDebugPanelContext combatDebugPanelContext;
            combatDebugPanelContext.isOpen = &showCombatPanel;
            combatDebugPanelContext.drawCombatShapesOverlay = &drawCombatShapesOverlay;
            combatDebugPanelContext.scene = &scene;
            combatDebugPanelContext.viewMatrix = cameraSystem.GetViewMatrix();
            combatDebugPanelContext.projectionMatrix = cameraSystem.GetProjectionMatrix();
            combatDebugPanelContext.viewportWidth = g_WindowWidth;
            combatDebugPanelContext.viewportHeight = g_WindowHeight;
            MyEngine::Editor::Panels::DrawCombatDebugPanel(combatDebugPanelContext);
        }
#endif
